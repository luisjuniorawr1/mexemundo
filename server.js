...
const server = http.createServer((req, res) => {
  ...
  if (requestUrl.pathname === '/health') {
    ...
    res.end(JSON.stringify({
      ...
      handSystem: 'universal-two-hand-v1.6.0',
      handInput: 'mexeflow-v2-anti-pull-visual-and-fast-collision',
      gamePostProcessing: 'none',
      menu: 'stable-five-second-dwell',
      startupGate: 'right-then-left-phone-sensor-calibration',
      sensorBinding: 'one-fixed-source-group-per-configured-hand',
      sensorOrder: 'right-then-left',
      wristVisibilityThreshold: 0.24,
      wristRecovery: 'calibrated-palm-offset-without-cross-hand-fallback',
      outlierProtection: 'hold-own-sensor-180ms-no-reassignment',
      gameStart: 'one-raised-hand-screen-only-adapter',
      training: 'once-per-phone-camera-session',
      profile: 'universal-runtime-configuration',
      tracking: 'pose-landmarker-lite-single-pass-reference-0.6.0',
      identity: 'sequential-calibrated-fixed-source-binding',
      singleHandAssignment: 'disabled-after-calibration',
      automaticHandSwap: 'disabled',
      crossHandFallback: 'disabled',
      visualResponse: 'mexeflow-v2-progressive-catch-up-no-hard-snap',
      gesture: 'not-required-for-menu-selection',
      shoulders: 'bound-with-each-configured-arm-group',
