...
const PATCHED = Symbol.for('mexemundo.rightHandGameUI');
...
export function resolveRightHandGameDwellMs(target, documentRef = document) {
...
export function selectRightControlHand(frame) {
...
export function shouldShowRightHandCursor({ target = null, targetOnly = false } = {}) {
  return !targetOnly || Boolean(target);
}

class RightHandGameCursor {
  constructor(element) {
  ...
  updateProgress(target, now) {
    ...
    const dwellMs = resolveRightHandGameDwellMs(target);
  ...
  updateFrame(frame, now = performance.now()) {
    ...
    const targetOnly = panelVisible(document, '#scoreHud')
      || panelVisible(document, '#countdownPanel');

    if (!shouldShowRightHandCursor({ target, targetOnly })) {
      this.lastAt = now;
      this.hide({ keepPosition: true });
      return;
    }
    ...
    this.render();
    this.updateProgress(target, now);
  }
}
...
export function startRightHandGameUI() {
  if (controller) return controller;

  const element = document.querySelector('#motionCursor');
  controller = new RightHandGameCursor(element);
