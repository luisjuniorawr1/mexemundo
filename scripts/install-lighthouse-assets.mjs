...
if (!input) {
  console.error('Uso: npm run assets:farol -- /caminho/farol-das-estrelas-hd.zip');
  console.error('Também é aceito o diretório já extraído farol-das-estrelas/.');
  process.exit(1);
}
...
function findKitRoot(root) {
  const directManifest = join(root, 'asset_manifest.json');
  if (existsSync(directManifest)) return root;

  const nested = join(root, 'farol-das-estrelas');
  if (existsSync(join(nested, 'asset_manifest.json'))) return nested;

  for (const entry of readdirSync(root)) {
    const candidate = join(root, entry);
    if (statSync(candidate).isDirectory() && existsSync(join(candidate, 'asset_manifest.json'))) return candidate;
  }

  throw new Error('Não encontrei a raiz do kit Farol das Estrelas.');
}
...
try {
  ...
  if (statSync(inputPath).isDirectory()) {
    ...
    extractedTemp = mkdtempSync(join(tmpdir(), 'mexemundo-farol-'));
    try {
